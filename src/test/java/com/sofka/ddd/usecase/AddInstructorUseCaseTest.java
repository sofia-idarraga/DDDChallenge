...
class AddInstructorUseCaseTest {
    ...
    void addInstructorTest() {
        ...
        AddInstructor command = new AddInstructor(courseID, instructorID, instructorName, email, specialty);
        AddInstructorUseCase useCase = new AddInstructorUseCase();
        ...
        var events = UseCaseHandler.getInstance()
                ...
                .orElseThrow(() -> new IllegalArgumentException("Something went wrong adding instructor"))
                .getDomainEvents();

        InstructorAdded event = (InstructorAdded) events.get(0);

        //Assert
        assertEquals("David Jaramillo", event.getName().value());
        assertEquals(COURSE_ID, event.aggregateRootId());
        assertEquals(instructorID.value(), event.getInstructorID().value());
        assertEquals("djaramillo@email.com", event.getEmail().value());
